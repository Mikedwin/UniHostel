...
router.delete('/applications/:id', auth, checkAdmin, async (req, res) => {
  try {
    const app = await Application.findById(req.params.id).populate('hostelId');
    if (!app) return res.status(404).json({ error: 'Application not found' });

    // If application was approved, decrease room occupancy
    if (app.status === 'approved' && app.hostelId) {
      const hostel = await Hostel.findById(app.hostelId._id);
      if (hostel) {
        const roomIndex = hostel.roomTypes.findIndex(r => r.type === app.roomType);
...
// TRASH ENDPOINTS
router.get('/trash/hostels', auth, checkAdmin, async (req, res) => {
  try {
    const hostels = await Hostel.find({ isDeleted: true })
...
router.patch('/trash/hostels/:id/restore', auth, checkAdmin, async (req, res) => {
  try {
    const hostel = await Hostel.findById(req.params.id);
    if (!hostel) return res.status(404).json({ error: 'Hostel not found' });
...
router.patch('/trash/users/:id/restore', auth, checkAdmin, async (req, res) => {
  try {
    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ error: 'User not found' });
