...
const EditHostelSimple = () => {
  ...
  useEffect(() => {
    const fetchHostel = async () => {
      try {
        ...
        setError(
          err.response?.data?.error || err.message || "Failed to load hostel",
        );
        setFetchLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen hostel-editor-page hostel-editor-page--edit bg-gray-50 py-8">
      <div className="max-w-3xl mx-auto px-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Edit Hostel</h1>
        <p className="text-gray-600 mb-8">
          Update your hostel information, room details, and pricing
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white p-6 rounded-lg shadow-sm space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mb-2">
                Upload a new image to replace the existing one (optional, max
                500KB)
              </p>
              <div className="flex items-center gap-4">
                ...
                {hostelViewImage &&
                  hostelViewImage.startsWith("data:image") && (
                    <button
                      type="button"
                      onClick={() => {
                        setHostelViewImage("");
              ...
              {hostelViewImage && hostelViewImage.startsWith("data:image") ? (
                ...
                <div className="mt-3 p-3 bg-gray-50 border border-gray-300 rounded-lg flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-700">
                    Existing image will be kept (not shown to avoid loading
                    delays)
                  </span>
                </div>
              ) : (
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Update pricing, capacity, and facilities for each room type
            </p>
            <div className="space-y-6">
              {roomTypes.map((room, index) => (
                <div
                  key={index}
                  className="border-2 border-gray-200 rounded-lg p-5 bg-gradient-to-br from-gray-50 to-white hover:border-blue-300 transition-colors"
                >
                              ...
                              return (
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    if (isSelected) {
                                      updated[index].facilities = updated[
                                        index
                                      ].facilities.filter(
                                        (f) => f !== facility,
                                      );
                    ...
                    {room.facilities && room.facilities.length > 0 && (
                      <div className="mt-3 p-3 bg-blue-50 rounded-md border border-blue-200">
                        <p className="text-xs font-medium text-blue-900 mb-2">
                          Selected Facilities ({room.facilities.length}):
                        </p>
                        <div className="flex flex-wrap gap-2">
