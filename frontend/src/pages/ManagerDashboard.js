...
const ManagerAnalytics = lazy(
  () => import("../components/manager/ManagerAnalytics"),
);
const ManagerTransactions = lazy(
  () => import("../components/manager/ManagerTransactions"),
);
...
const ManagerDashboard = () => {
  ...
  const handleContextMenu = (event, application) => {
    ...
    if (
      viewMode === "active" &&
      ["approved", "rejected", "approved_for_payment"].includes(
        application.status,
      )
    ) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [appRes, hostRes] = await Promise.all([
        axios.get(
          `${API_ENDPOINTS.MANAGER_APPLICATIONS}?archived=${archived}`,
          {
  ...
  const handleArchive = async (id, archive) => {
    try {
      ...
      showToast(
        archive ? "Moved to history successfully" : "Restored successfully",
      );
      fetchData();
    } catch (err) {
      console.error(err);
  ...
  const handleStatusUpdate = async (id, action) => {
    try {
      ...
      if (action === "final_approve" && response.data.accessCode) {
        showToast(
          `Application approved! Access Code: ${response.data.accessCode}`,
        );
  ...
  const handleBulkAction = async (action) => {
    ...
    const actionText =
      action === "approve_for_payment" ? "Approve for Payment" : "Reject";
  ...
  const stats = useMemo(() => {
    ...
    const totalCapacity = hostels.reduce(
      (sum, hostel) =>
        ...
        hostel.roomTypes.reduce(
          (roomSum, room) => roomSum + room.totalCapacity,
          0,
        ),
      0,
    );
    const totalOccupied = hostels.reduce(
      (sum, hostel) =>
        sum +
        hostel.roomTypes.reduce(
          (roomSum, room) => roomSum + (room.occupiedCapacity || 0),
          0,
        ),
      0,
    );
  ...
  return (
    <div className="min-h-screen manager-workspace bg-[linear-gradient(180deg,#f3fbf9_0%,#ffffff_24%,#f8fbfb_100%)]">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {userInfo &&
          !userInfo.isVerified &&
          userInfo.accountStatus === "pending_verification" && (
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  ...
                  <div className="mt-2 text-sm text-yellow-700">
                    <p>
                      Your manager account is awaiting admin approval. You
                      cannot create or manage hostels until your account is
        ...
        <ManagerContextMenu
          ...
          onArchiveFromContext={handleArchiveFromContext}
        />
      </div>
    </div>
  );
