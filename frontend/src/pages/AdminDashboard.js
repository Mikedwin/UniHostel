...
const ApplicationManagementTable = lazy(
  () => import("../components/admin/ApplicationManagementTable"),
);
...
const ManagerRegistrationForm = lazy(
  () => import("../components/admin/ManagerRegistrationForm"),
);
const VisitorTracking = lazy(
  () => import("../components/admin/VisitorTracking"),
);
const WaitlistManagementTable = lazy(
  () => import("../components/admin/WaitlistManagementTable"),
);
...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [statsRes, hostelsRes, managersRes, logsRes] = await Promise.all([
        ...
        axios.get(`${API_URL}/api/admin/logs?limit=20`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
  ...
  const fetchHistoryLogs = async () => {
    try {
      const res = await axios.get(
        `${API_URL}/api/admin/logs/history?limit=50`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const sendTestEmail = async () => {
    try {
      ...
      showError(
        err.response?.data?.error || "Unable to send a test email right now",
      );
    } finally {
  ...
  const flagHostel = async (hostelId) => {
    ...
    try {
      await axios.patch(
        `${API_URL}/api/admin/hostels/${hostelId}/flag`,
        { reason },
        { headers: { Authorization: `Bearer ${token}` } },
      );
      fetchDashboardData();
  ...
  const handleBulkAction = async (action, userIds) => {
    ...
    try {
      const res = await axios.post(
        `${API_URL}/api/admin/users/bulk-action`,
        {
          userIds,
          action,
          reason,
    ...
    showConfirm(
      ...
      async () => {
        try {
          ...
          showError(
            err.response?.data?.error || "Failed to delete application",
          );
        }
      },
    );
  ...
  const handleBulkApplicationAction = async (
    ...
    try {
      const res = await axios.post(
        `${API_URL}/api/admin/applications/bulk-action`,
        {
  ...
  return (
    <div className="min-h-screen admin-workspace bg-[linear-gradient(180deg,#f3fbf9_0%,#ffffff_24%,#f8fbfb_100%)]">
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="mb-6 rounded-[2rem] border border-emerald-100/70 bg-[linear-gradient(135deg,#ffffff_0%,#f3fbf9_100%)] p-6 shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-bold uppercase tracking-[0.24em] text-primary-600">
                Admin dashboard
              </p>
              <h1 className="mt-3 text-3xl font-black tracking-tight text-slate-950">
                System oversight, approvals, and platform control.
              </h1>
              <p className="mt-2 text-sm text-slate-600">
                Manage users, hostels, applications, analytics, and email
                security from one cleaner control surface.
              </p>
            </div>
            <Link
              to="/change-password"
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-600">
                Confirm email delivery here before turning privileged MFA back
                on.
              </p>
              <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-gray-700">
                ...
                <span>
                  App password format:{" "}
                  {emailStatus?.passwordLooksLikeAppPassword
                    ? "Looks valid"
                    : "Check it"}
                </span>
              </div>
              {!emailStatus?.verified && (
                <p className="text-sm text-amber-700">
                  If the test fails, re-copy the Gmail App Password into Render.
                  Google often shows it in groups, but the app now ignores
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              ...
              >
                <RefreshCw
                  className={`h-4 w-4 ${emailStatusLoading ? "unihostel-button-loader" : ""}`}
                />
                Refresh Status
              </button>
              <button
        ...
        <div className="bg-white rounded-[1.75rem] border border-white/70 shadow-[0_18px_40px_rgba(15,23,42,0.06)] mb-6">
          ...
          <div className="p-3 sm:p-4">
            <AdminTabContent
              ...
              WaitlistManagementTable={WaitlistManagementTable}
