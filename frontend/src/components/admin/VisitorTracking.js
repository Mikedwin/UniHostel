...
import { Eye, Monitor, Smartphone, Tablet, Globe } from "lucide-react";
import { FilterSearchInput, FilterDateInput, FilterButton, FilterBar } from '../DashboardFilters';
...
const VisitorTracking = () => {
  ...
  const fetchVisitors = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/admin/visitors?${params}`,
        {
  ...
  const fetchStats = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/admin/visitors/stats?${params}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  return (
    <div className="operations-surface visitor-tracking space-y-6">
      {/* Header */}
      ...
      <FilterBar title="Search & Filter">
        ...
        <div className="flex items-end">
          <FilterButton variant="primary" onClick={handleSearch} className="w-full">
            Search
          </FilterButton>
        </div>
      </FilterBar>
