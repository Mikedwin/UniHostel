...
const AdminTransactions = ({ token }) => {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const res = await axios.get(
        `${API_URL}/api/transactions/admin?${params}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const handleResetTransactions = async () => {
    ...
    try {
      const res = await axios.delete(
        `${API_URL}/api/transactions/admin/reset`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  return (
    <div className="operations-surface admin-transactions space-y-6">
      {/* Revenue Summary */}
        ...
        <div className="bg-gradient-to-br from-green-50 to-white rounded-lg shadow-md p-5 border border-green-100">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-1">
                GH₵{summary?.totalAdminCommission?.toLocaleString() || 0}
              </p>
            </div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-white rounded-lg shadow-md p-5 border border-purple-100">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-1">
                GH₵{summary?.totalHostelRevenue?.toLocaleString() || 0}
              </p>
            </div>
      ...
      {/* Filters */}
      <FilterBar title="Filter Transactions" activeCount={[filters.semester, filters.hostelId, filters.managerId, filters.startDate, filters.endDate].filter(Boolean).length} onReset={clearFilters}>
        <FilterSelect
          label="Semester"
