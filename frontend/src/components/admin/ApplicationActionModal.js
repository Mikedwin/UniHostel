...
const ApplicationActionModal = ({
    ...
    switch (action) {
      case "approve":
        return {
          ...
          color: "green",
          message:
            "This will approve the application and update room capacity.",
          requireReason: true,
        };
      case "reject":
        return {
          ...
          color: "red",
          message:
            "This will reject the application. If previously approved, capacity will be adjusted.",
          requireReason: true,
        };
      case "note":
        return {
      ...
      case "resolve-dispute":
        return {
          ...
          color: "blue",
          message:
            "Provide resolution details and optionally change application status.",
  ...
  return (
    <div className="operations-modal fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-md w-full">
        <div
          className={`px-6 py-4 border-b flex justify-between items-center bg-${config.color}-50`}
        >
          <h2 className={`text-lg font-bold text-${config.color}-900`}>
            {config.title}
          </h2>
          <button
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="p-6">
          {config.message && (
            <div
              className={`mb-4 p-3 bg-${config.color}-50 border border-${config.color}-200 rounded flex gap-2`}
            >
              <AlertTriangle
                className={`h-5 w-5 text-${config.color}-600 flex-shrink-0 mt-0.5`}
              />
          ...
          <div className="mb-4 p-3 bg-gray-50 rounded text-sm">
            ...
            <p>
              <span className="font-medium">Room:</span> {application?.roomType}
            </p>
          </div>
              ...
              <div className="mb-4">
                <label className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-700">
                    Make visible to hostel manager
                  </span>
                </label>
              </div>
            </>
            ...
            <>
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, disputeReason: e.target.value })
                  }
                  className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
                  required
              ...
              <div className="mb-4">
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, disputeDetails: e.target.value })
                  }
                  className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
                  rows="3"
